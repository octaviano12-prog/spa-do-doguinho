...
const fallbackServices = [
  { id: "banho", name: "Banho & Tosa", description: "Higiene completa, pelagem bem cuidada e finalização cheia de charme.", image_url: img("service-banho.jpg"), icon: Scissors },
  { id: "spa", name: "Spa Relaxante", description: "Momento tranquilo para o pet relaxar e se sentir acolhido.", image_url: img("service-spa.jpg"), icon: Heart },
  { id: "agenda", name: "Agendamento Online", description: "Escolha serviço, data e horário pelo site de forma rápida.", image_url: img("service-agendamento.jpg"), icon: CalendarCheck },
];
...
export default function PublicHome() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try { const { data } = await client.get("/services"); setServices(Array.isArray(data) ? data.filter((s) => Number(s.active) !== 0) : []); }
      catch { setServices([]); }

      try { const { data } = await client.get("/gallery"); setGallery(Array.isArray(data) ? data : []); }
      catch { setGallery([]); }
    }
  ...
  const serviceList = useMemo(() => services.length
    ? services.slice(0,3).map((s,i) => ({ ...s, icon: fallbackServices[i]?.icon || Scissors, image_url: s.image_url || fallbackServices[i]?.image_url }))
    : fallbackServices
  , [services]);
  ...
  return (
    <div className="publicSite">
      {/* WhatsApp Flutuante */}
      <a href={whatsappHref} target="_blank" rel="noreferrer" className="whatsappFloat"><MessageCircle size={30} /></a>

      {/* Header */}
      <header className="publicHeader">
        <Link to="/" className="publicLogo"><div className="brandIcon"><Dog /></div>
        ...
        <button onClick={goBooking} className="btn gold publicHeaderCta"><CalendarCheck size={18} /> Agendar</button>
      </header>
      ...
      <section className="publicHero">
        ...
        <motion.div className="heroContent" initial={{ opacity:0, y:28 }} animate={{ opacity:1, y:0 }}>
          <div className="heroBadge"><Star size={16} /> Referência em estética pet</div>
          <h1>Cuidado, carinho e beleza para o seu <span>melhor amigo</span></h1>
          <p>Banho, tosa, vacinas e relaxamento com atendimento premium, carinho de verdade e agendamento online.</p>

          <div className="heroButtons">
            <button onClick={goBooking} className="btn gold"><CalendarCheck size={18} /> Agendar agora</button>
            <a href={whatsappHref} target="_blank" rel="noreferrer" className="btn ghost"><Phone size={18} /> Falar no WhatsApp</a>
          </div>
        ...
        <motion.div className="heroImageWrap" initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.7 }}>
          ...
          <div className="heroFloatingCard">
            <div className="heroStars">{[1,2,3,4,5].map((i) => <Star key={i} size={16} fill="currentColor" />)}</div>
      ...
      <section className="publicStats">
        {[
          ...
          <div className="statBox" key={value}><Icon size={26} /><strong>{value}</strong><span>{label}</span></div>
        ))}
      </section>

      {/* Serviços */}
      <section className="servicesSection" id="servicos">
        <div className="sectionTitle"><span><Sparkles size={16} /> Serviços especiais</span><h2>Tudo que seu pet precisa em um só lugar</h2></div>
        ...
        <div className="aboutPremiumGrid">
          <div className="aboutBox card">
            <div>
              ...
              <p>O SPA do Doguinho une visual profissional, agendamento fácil, carinho, segurança e uma experiência premium para encantar clientes.</p>
            </div>
            <div className="aboutChecklist">
              {["Ambiente climatizado e acolhedor.","Produtos especiais para pele e pelagem.","Atendimento com carinho de verdade.","Profissionais experientes."].map((text) => (
                <div key={text}><CheckCircle2 size={20} /><span>{text}</span></div>
              ))}
            </div>
          </div>
      ...
      <section className="testimonialsSection" id="depoimentos">
        <div className="sectionTitle"><span>Depoimentos</span><h2>Quem conhece recomenda</h2></div>
          ...
          <div className="testimonialCards">
            {[
              ...
              <div className="card testimonialCard" key={name}>
                ...
                <p>“{text}”</p>
                <strong>{name}</strong>
                <div className="heroStars">{[1,2,3,4,5].map((i)=><Star key={i} size={14} fill="currentColor" />)}</div>
              </div>
            ))}
          </div>
      ...
      <section className="servicesSection" id="galeria-preview">
        <div className="sectionTitle"><span>Galeria</span><h2>Momentos especiais no SPA</h2></div>
        <div className="homeGalleryGrid">
          ...
          ]).map((item)=>(<div className="homeGalleryItem" key={item.id}><img src={item.image_url} alt={item.title||"Galeria SPA do Doguinho"} /></div>))}
        </div>
      </section>
