...
export default function GaleriaPage() {
  ...
  async function saveItem(event) {
    ...
    setError("");
    try {
      await apiRequest("/gallery", { method: "POST", body: JSON.stringify(form) });
      setForm(emptyForm);
      await loadGallery();
    } catch (err) {
      ...
      <div className="space-y-8">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div>
            ...
            <p className="text-green-100/80 mt-3 max-w-3xl">Cadastre imagens, resultados e registros do SPA para aparecerem na página pública de galeria.</p>
          </div>
        ...
        <div className="grid xl:grid-cols-[430px_1fr] gap-6">
          <form onSubmit={saveItem} className="glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
            <div className="flex items-center gap-4 mb-7">
              ...
              <div><h2 className="text-2xl font-black text-slate-900">Nova imagem</h2><p className="text-slate-500">Use URL da imagem por enquanto.</p></div>
            </div>
            <div className="grid gap-4">
              ...
              <input value={form.image_url} onChange={(event) => updateForm("image_url", event.target.value)} placeholder="URL da imagem" className="bg-white border border-slate-200 rounded-2xl px-5 py-4 outline-none text-slate-900" />
              ...
              {form.image_url && <div className="h-[180px] rounded-3xl bg-cover bg-center border border-slate-200 overflow-hidden" style={{ backgroundImage: `url('${form.image_url}')` }} />}
              <button type="submit" disabled={saving} className="bg-green-600 hover:bg-green-700 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 transition disabled:opacity-60"><Sparkles size={20} /> {saving ? "Salvando..." : "Publicar na galeria"}</button>
            </div>
          </form>
