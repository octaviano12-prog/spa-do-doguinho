...
export default function ServicosPage() {
  ...
  const summary = useMemo(() => {
    ...
    const avgDuration = services.length
      ? services.reduce((sum, item) => sum + Number(item.duration_minutes || 0), 0) / services.length
      : 0;
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div>
            ...
            <p className="text-green-100/80 mt-3 max-w-3xl">
              Gerencie banho, tosa, vacinas, spa e pacotes exibidos no site público e usados no agendamento.
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-4 gap-6">
          {[
            ["Serviços", summary.total, Sparkles],
            ["Ativos", summary.active, PawPrint],
            ["Preço médio", formatCurrency(summary.avgPrice), DollarSign],
            ["Duração média", `${Math.round(summary.avgDuration || 0)} min`, Clock]
        ...
        <div className="grid xl:grid-cols-[430px_1fr] gap-6">
          <form onSubmit={handleCreate} className="glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
            <div className="flex items-center gap-4 mb-7">
              ...
              <div>
                ...
                <p className="text-slate-500">Aparece no site público e no agendamento.</p>
              </div>
            </div>
            ...
            <div className="grid gap-4">
              ...
              <select value={form.category} onChange={(e) => updateForm("category", e.target.value)} className="bg-white border border-slate-200 rounded-2xl px-5 py-4 outline-none text-slate-900">
                <option>Banho</option>
                <option>Tosa</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="number" step="0.01" value={form.price} onChange={(e) => updateForm("price", e.target.value)} placeholder="Preço" className="bg-white border border-slate-200 rounded-2xl px-5 py-4 outline-none text-slate-900" />
                <input type="number" value={form.duration_minutes} onChange={(e) => updateForm("duration_minutes", Number(e.target.value))} placeholder="Duração" className="bg-white border border-slate-200 rounded-2xl px-5 py-4 outline-none text-slate-900" />
              </div>
          ...
          <div className="glass rounded-[32px] p-6 border border-white/30 shadow-2xl">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {loading && [1, 2, 3, 4].map((item) => <div key={item} className="h-[290px] rounded-3xl bg-white/50 animate-pulse" />)}

              {!loading && filteredServices.length === 0 && (
                <div className="md:col-span-2 bg-white rounded-2xl p-8 text-center text-slate-500 border">
              ...
              {!loading && filteredServices.map((service) => {
                ...
                return (
                  <div key={service.id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-slate-900 mt-5">{service.name}</h3>
                    <p className="text-slate-500 mt-3 min-h-[70px]">{service.description || "Sem descrição."}</p>
                    <div className="grid grid-cols-2 gap-4 mt-5">
                      <div className="bg-slate-50 rounded-2xl p-4">
                        <div className="text-xs text-slate-400 font-bold">PREÇO</div>
                        <div className="font-black text-green-700">{formatCurrency(service.price)}</div>
                      </div>
                      <div className="bg-slate-50 rounded-2xl p-4">
                        <div className="text-xs text-slate-400 font-bold">DURAÇÃO</div>
                        <div className="font-black text-slate-900">{service.duration_minutes || 60} min</div>
                      </div>
                    </div>
                    {service.benefits && (
                      <div className="bg-green-50 rounded-2xl p-4 mt-4 text-green-900 text-sm">
                        <strong>Benefícios:</strong> {service.benefits}
                      </div>
                    )}
                    <button onClick={() => handleDelete(service.id)} className="mt-5 w-full bg-red-50 hover:bg-red-100 text-red-700 py-3 rounded-2xl font-black flex items-center justify-center gap-2 transition">
                      <Trash2 size={18} />
                      Excluir
                    </button>
                  </div>
