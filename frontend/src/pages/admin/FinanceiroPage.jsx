...
export default function FinanceiroPage() {
  ...
  const summary = useMemo(() => {
    ...
    const cashIn = cash
      .filter((item) => ["in", "entrada", "income"].includes(normalize(item.type)))
      .reduce((sum, item) => sum + Number(item.amount || 0), 0);

    const cashOut = cash
      .filter((item) => ["out", "saida", "expense"].includes(normalize(item.type)))
      .reduce((sum, item) => sum + Number(item.amount || 0), 0);
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div>
            ...
            <p className="text-green-100/80 mt-3 max-w-3xl">
              Visão geral das entradas, pagamentos pendentes, caixa e movimentações do SPA do Doguinho.
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
          {[
            ["Recebido", formatCurrency(summary.paid), CheckCircle, "bg-green-600"],
            ["Pendente", formatCurrency(summary.pending), Clock, "bg-yellow-500"],
            ["Saídas", formatCurrency(summary.cashOut), ArrowDownCircle, "bg-red-600"],
            ["Saldo", formatCurrency(summary.balance), Wallet, "bg-emerald-700"]
            ...
            <div key={label} className="glass rounded-[30px] p-6 border border-white/30 shadow-2xl">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className={`w-16 h-16 rounded-2xl ${color} text-white flex items-center justify-center`}>
                  <Icon size={30} />
                </div>
              </div>
            </div>
        ...
        <div className="grid xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
            <div className="flex items-center gap-3 mb-7">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-900">Resumo operacional</h2>
                <p className="text-slate-500">Indicadores principais do período atual.</p>
              </div>
            </div>

            <div className="grid md:grid-cols-3 gap-5">
              <div className="bg-white rounded-3xl p-6 border border-slate-100">
                <ArrowUpCircle className="text-green-600 mb-4" size={34} />
                <div className="text-slate-500 font-bold">Entradas de caixa</div>
                <div className="text-2xl font-black text-slate-900 mt-2">{formatCurrency(summary.cashIn)}</div>
              </div>
              <div className="bg-white rounded-3xl p-6 border border-slate-100">
                <DollarSign className="text-green-600 mb-4" size={34} />
                <div className="text-slate-500 font-bold">Total financeiro</div>
                <div className="text-2xl font-black text-slate-900 mt-2">{formatCurrency(summary.total)}</div>
              </div>
              <div className="bg-white rounded-3xl p-6 border border-slate-100">
                <CalendarDays className="text-green-600 mb-4" size={34} />
                <div className="text-slate-500 font-bold">Agendamentos</div>
                <div className="text-2xl font-black text-slate-900 mt-2">{summary.appointments}</div>
              </div>
            </div>
          </div>
          ...
          <div className="glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item} className="bg-white rounded-2xl p-4 border border-slate-100 font-bold text-slate-700">
                  {item}
                </div>
              ))}
            </div>
        ...
        <div className="glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            {!loading && recentPayments.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl p-5 border border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <div className="font-black text-slate-900">Pagamento #{item.id}</div>
                  <div className="text-slate-500 mt-1">
                    Método: {item.method || item.payment_method || "Não informado"} • Status: {item.status || item.payment_status || "Pendente"}
                  </div>
                </div>
