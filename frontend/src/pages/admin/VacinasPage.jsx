    ...
    <AdminLayout>
      <div className="space-y-8">
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div>
            ...
            <p className="text-green-100/80 mt-3 max-w-3xl">
              Controle aplicações, próximas doses, pets em atraso e lembretes para tutores.
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-4 gap-6">
          {[
            ["Registros", summary.total, Syringe, "bg-green-600"],
            ["Em dia", summary.ok, CheckCircle, "bg-emerald-700"],
            ["Próx. 30 dias", summary.dueSoon, Clock, "bg-yellow-500"],
            ["Atrasadas", summary.overdue, AlertTriangle, "bg-red-600"]
        ...
        <div className="glass rounded-[32px] p-6 border border-white/30 shadow-2xl">
          <label className="relative block">
            ...
            <input
              ...
              placeholder="Buscar por vacina, pet, tutor ou status..."
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
                    ...
                    <div className="grid xl:grid-cols-[1fr_auto] gap-5 items-center">
                      <div className="grid md:grid-cols-4 gap-5">
                        <div className="flex gap-3">
                          <Syringe className="text-green-600 shrink-0" />
                          <div>
                            <div className="text-xs text-slate-400 font-bold uppercase">Vacina</div>
                            <div className="font-black text-slate-900">{item.vaccine_name || item.name || "Vacina"}</div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                          ...
                          <div>
                            <div className="text-xs text-slate-400 font-bold uppercase">Próxima dose</div>
                            <div className="font-black text-slate-900">{formatDate(item.next_dose_date || item.next_date)}</div>
                          </div>
                        </div>
                      </div>

                      <div className="flex xl:justify-end">
                        <span className={`px-4 py-3 rounded-2xl font-black ${isOverdue ? "bg-red-100 text-red-700" : isDueSoon ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
                          {isOverdue ? "Atrasada" : isDueSoon ? "Próxima" : "Em dia"}
                        </span>
                      </div>
                    </div>

                    <div className="mt-5 bg-slate-50 rounded-2xl p-4 text-slate-600">
                      <strong>Aplicação:</strong> {formatDate(item.date || item.applied_at)}
          ...
          <div className="glass rounded-[32px] p-8 border border-white/30 shadow-2xl">
            ...
            <p className="text-slate-500 mt-3">
              Esta tela está preparada para enviar lembretes de próximas doses por WhatsApp ou notificação.
            </p>
