...
export default function DashboardPage() {
  ...
  const todayAppointments = useMemo(() => {
    ...
    return data.appointments.filter((item) => String(item.date || item.scheduled_at || "").slice(0, 10) === today).length;
  }, [data.appointments]);

  const cards = [
    { title: "Clientes", value: data.customers.length, icon: Users },
    { title: "Pets", value: data.pets.length, icon: PawPrint },
    { title: "Hoje", value: todayAppointments, icon: CalendarDays },
    { title: "Faturamento", value: formatCurrency(summary.revenue), icon: Wallet }
  ];

  const actions = [
    { title: "Agenda", text: "Gerenciar atendimentos", to: "/admin/agendamentos", icon: CalendarDays },
    { title: "Clientes", text: "Novo tutor", to: "/admin/clientes", icon: Users },
    { title: "Serviços", text: "Preços e duração", to: "/admin/servicos", icon: Scissors },
    { title: "Disponibilidade", text: "Horários e bloqueios", to: "/admin/disponibilidade", icon: Clock },
    { title: "Pagamentos", text: "PIX e financeiro", to: "/admin/financeiro", icon: CreditCard },
    { title: "Estoque", text: "Produtos e alertas", to: "/admin/estoque", icon: Wallet }
  ];
