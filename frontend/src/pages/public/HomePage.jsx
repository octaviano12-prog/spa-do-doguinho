  ...
  return (
    <div className={`relative flex min-h-[220px] items-center justify-center overflow-hidden rounded-[24px] border border-dashed border-[#0d6b54]/25 bg-[linear-gradient(135deg,#e6f5eb,#fff8e6)] p-6 text-center ${className}`}>
      ...
      <div className="relative">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-[22px] bg-white text-[#0d6b54] shadow-xl">
          <PawPrint size={30} />
        </div>
      ...
      <main className="overflow-hidden bg-[#fffdf7] text-[#12382f]">
          ...
          <div className="mx-auto max-w-[1500px]">
            ...
            <div className="grid gap-5 md:grid-cols-4">
              {services.map(({ icon: Icon, title, text, price }) => (
                <div key={title} className="rounded-[26px] bg-white p-7 text-center shadow-xl ring-1 ring-[#e2eadf] transition hover:-translate-y-1 hover:shadow-2xl">
                  <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#e7f4ed] text-[#0d6b54]"><Icon size={38} /></div>
        ...
        <section className="px-5 py-16 md:px-8">
          <div className="mx-auto max-w-[1300px]">
            <h2 className="text-center text-3xl font-black text-[#0d6b54] md:text-4xl">Por que escolher o SPA do Doguinho?</h2>
            <div className="mt-10 grid gap-8 md:grid-cols-3">
              {benefits.map(([title, text, Icon]) => (
                <div key={title} className="flex items-center gap-5 rounded-[26px] bg-white p-6 shadow-sm ring-1 ring-[#e2eadf]">
                  <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-[#e7f4ed] text-[#0d6b54]"><Icon size={38} /></div>
        ...
        <section className="px-5 pb-16 md:px-8">
          <div className="mx-auto grid max-w-[1400px] overflow-hidden rounded-[30px] bg-white shadow-xl ring-1 ring-[#e2eadf] lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className="p-8 md:p-10">
              ...
              <div className="mt-7 grid gap-4 md:grid-cols-3">
                {["Escolher pet", "Porte do cachorro", "Serviço", "Data", "Horário"].map((label) => (
                  <div key={label} className="rounded-2xl border border-[#e2eadf] bg-white px-5 py-4 shadow-sm">
                    <div className="text-xs font-bold text-slate-500">{label}</div>
                    <div className="mt-2 flex items-center justify-between font-black text-[#12382f]">Selecionar <ArrowRight size={16} /></div>
                  </div>
                ))}
                <Link to="/agendamento" className="flex items-center justify-center gap-2 rounded-2xl bg-[#0d6b54] px-5 py-4 font-black text-white shadow-lg transition hover:bg-[#095642]"><CalendarDays size={18} /> Fazer agendamento</Link>
              </div>
        ...
        <section className="px-5 pb-16 md:px-8">
          <div className="mx-auto max-w-[1400px] text-center">
            <h2 className="text-3xl font-black text-[#0d6b54] md:text-4xl">Nossa galeria</h2>
            <div className="mt-5 flex flex-wrap justify-center gap-2">{["Todos", "Banho", "Tosa", "Antes e Depois", "Clientes"].map((filter) => <span key={filter} className="rounded-full bg-[#edf2ec] px-5 py-2 text-sm font-black text-slate-600">{filter}</span>)}</div>
            <div className="mt-8 grid gap-5 md:grid-cols-4">{[1, 2, 3, 4].map((item) => <ImagePlaceholder key={item} title={`Imagem ${item}`} />)}</div>
          </div>
        </section>

        <section className="bg-[#e7f4ed] px-5 py-16 md:px-8">
          <div className="mx-auto max-w-[1400px]">
            <h2 className="text-center text-3xl font-black text-[#0d6b54] md:text-4xl">O que dizem nossos clientes</h2>
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {testimonials.map(([text, name, role]) => (
                <div key={text} className="rounded-[24px] bg-white p-7 shadow-lg">
        ...
        <section className="px-5 py-12 md:px-8">
          <div className="mx-auto flex max-w-[1500px] flex-col gap-6 rounded-t-[90px] rounded-b-[28px] bg-[#0d6b54] p-8 text-white shadow-2xl md:flex-row md:items-center md:justify-between md:p-10">
            <div className="flex items-center gap-6">
              <div className="flex h-24 w-24 items-center justify-center rounded-full border-4 border-white/70"><PawPrint size={50} /></div>
