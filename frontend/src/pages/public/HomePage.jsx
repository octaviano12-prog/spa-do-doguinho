    ...
    <PublicLayout>
      <main className="overflow-hidden bg-[#edf6ed] text-[#102d27]">
        <section className="relative overflow-hidden bg-[#dcefe3] px-5 pb-16 pt-10 md:px-8 2xl:px-10">
          ...
          <div className="relative mx-auto grid max-w-[1680px] gap-10 xl:grid-cols-[.95fr_1.05fr] xl:items-center">
            <div className="max-w-4xl py-10 xl:py-20">
              ...
              <div className="mt-9 flex flex-wrap gap-4">
                ...
                <a href="https://wa.me/5518997493722" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 rounded-2xl border border-[#b7d7c2] bg-white px-7 py-4 font-black text-[#102d27] shadow-sm transition hover:-translate-y-0.5 hover:border-[#19534e]">
                  <MessageCircle size={19} /> WhatsApp
                </a>
              </div>

              <div className="mt-10 grid max-w-2xl gap-3 sm:grid-cols-3">
                {[["+3.500", "pets atendidos"], ["5★", "experiência premium"], ["online", "agenda fácil"]].map(([value, label]) => (
                  <div key={label} className="rounded-3xl bg-white/92 p-5 shadow-lg ring-1 ring-[#b7d7c2]/40">
            ...
            <div className="relative min-h-[680px]">
              <div className="absolute right-0 top-0 h-[620px] w-[86%] overflow-hidden rounded-[58px] bg-slate-200 shadow-2xl ring-1 ring-black/5">
                <img src={photos.hero} alt="Pet após banho" className="h-full w-full object-cover" />
              </div>

              <div className="absolute bottom-0 left-0 w-[46%] overflow-hidden rounded-[38px] border-[10px] border-[#dcefe3] bg-white shadow-2xl">
                <img src={photos.bath} alt="Banho cuidadoso" className="h-72 w-full object-cover" />

                <div className="p-5">
                  <div className="flex gap-1 text-[#f4c86a]">{[1,2,3,4,5].map((i)=><Star key={i} size={16} fill="currentColor" />)}</div>
              ...
              <div className="absolute right-8 bottom-14 rounded-[28px] bg-white/92 p-5 shadow-2xl backdrop-blur">
                <div className="flex items-center gap-4">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e6f3ef] text-[#19534e]"><ShieldCheck size={28} /></div>
                  <div>
        ...
        <section className="mx-auto grid max-w-[1680px] gap-5 px-5 py-12 md:grid-cols-4 md:px-8 2xl:px-10">
          {[[Bath, "Banho Premium"], [Scissors, "Tosa Boutique"], [Sparkles, "Spa Relaxante"], [Syringe, "Vacinação"]].map(([Icon, title]) => (
            <div key={title} className="rounded-[30px] bg-white p-7 shadow-xl ring-1 ring-[#b7d7c2]/40 transition hover:-translate-y-1">
              <Icon className="text-[#19534e]" size={34} />
              <h3 className="mt-5 text-2xl font-black">{title}</h3>
              <p className="mt-2 text-slate-600">Atendimento premium e cuidadoso para seu pet.</p>
            </div>
          ))}
        </section>

        <section className="px-5 pb-16 md:px-8 2xl:px-10">
          <div className="mx-auto grid max-w-[1680px] gap-8 rounded-[42px] bg-[#19534e] p-8 text-white shadow-2xl md:grid-cols-2 md:p-12 xl:items-center">
            <div>
              <span className="inline-flex rounded-full bg-white/10 px-5 py-2 text-sm font-black text-[#dcefe3]">Pacotes por porte</span>
              <h2 className="mt-5 text-4xl font-black leading-tight md:text-6xl">Preço claro antes de agendar.</h2>
              <p className="mt-5 max-w-xl text-lg leading-relaxed text-white/75">Valores separados por porte do pet para facilitar o atendimento.</p>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              {[["Pequeno", "R$ 60", "~1h"], ["Médio", "R$ 80", "~1h30"], ["Grande", "R$ 100", "~2h"], ["Gigante", "R$ 120", "~2h30"]].map(([name, price, time]) => (
                <div key={name} className="rounded-[30px] bg-white/10 p-6 ring-1 ring-white/10 backdrop-blur">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-white/70"><Clock size={18} />{time}</div>
                </div>
              ))}
            </div>
          </div>
