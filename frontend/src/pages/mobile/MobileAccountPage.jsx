...
export default function MobileAccountPage() {
  ...
  return (
    <MobileShell title="Meu Painel" showBack={false} active="home">
      <section className="relative mx-3 mt-3 overflow-hidden rounded-[36px] bg-gradient-to-br from-[#0d6b54] via-[#1b7f52] to-[#14532d] px-5 pb-6 pt-7 text-white shadow-xl">
        <div className="absolute -right-12 -top-12 h-44 w-44 rounded-full bg-white/15 blur-3xl" />
        <div className="absolute -bottom-10 -left-10 h-40 w-40 rounded-full bg-black/15 blur-2xl" />

        <div className="relative z-10 mb-7 flex items-center gap-4">
          <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border border-white/30 bg-white/15 text-white shadow-lg ring-4 ring-white/10">
            <UserRound size={38} />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white/80">Bem-vindo de volta</p>
            <h1 className="truncate text-3xl font-black leading-tight">Olá, {firstName}!</h1>
      ...
      <main className="space-y-6 px-4 py-6">
        ...
        {nextBooking ? (
          <section className="relative overflow-hidden rounded-[32px] border border-black/5 bg-white p-5 shadow-lg">
            <div className="absolute right-0 top-0 h-32 w-32 rounded-full bg-[#0d6b54]/20 blur-3xl" />
            <div className="relative z-10 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-4 text-lg font-black text-[#0d6b54]">
              ...
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#0d6b54] text-white shadow-lg">
                <CalendarDays size={28} />
              </div>
            </div>
          </section>
        ) : (
          <section className="rounded-[32px] border border-black/5 bg-white p-5 shadow-md">
...
function StatBox({ icon: Icon, label, value }) {
  return (
    <div className="rounded-[26px] border border-white/20 bg-white/15 p-4 shadow-sm backdrop-blur-xl">
...
function ActionCard({ to, href, icon: Icon, title, subtitle, colorClass }) {
  const content = (
    <div className="group flex h-full min-h-[112px] flex-col justify-between rounded-[28px] border border-black/5 bg-white p-4 text-left shadow-md transition active:scale-95">
