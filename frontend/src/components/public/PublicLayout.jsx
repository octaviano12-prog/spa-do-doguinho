...
function BrandLogo({ small = false }) {
  const size = small ? "w-12 h-12" : "w-[54px] h-[54px]";
  return (
    <div className={`${size} relative shrink-0 rounded-[20px] overflow-hidden bg-gradient-to-br from-[#effff9] via-[#cbfff0] to-[#18b38b] border border-teal-200/80 shadow-[0_0_22px_rgba(20,184,166,.28)] flex items-center justify-center`}>
      <div className="absolute inset-[5px] rounded-[16px] bg-white/86" />
      <div className="absolute top-2 text-[#071924]"><PawPrint size={small ? 21 : 23} /></div>
  ...
  return (
    <div className="min-h-screen bg-[#050f0b] text-white overflow-x-hidden">
      <header className="fixed top-0 left-0 right-0 z-50 px-3 md:px-6 pt-3">
        <div className="max-w-[1680px] mx-auto rounded-[24px] border border-white/10 bg-[#03130d]/88 shadow-[0_18px_55px_rgba(0,0,0,.42)] backdrop-blur-2xl overflow-hidden">
          ...
          <div className="relative min-h-[66px] px-4 2xl:px-5 flex items-center justify-between gap-3">
            <Link to="/" className="group flex items-center gap-3 min-w-0 shrink-0 w-[235px] xl:w-[245px] 2xl:w-[270px]" onClick={() => setMobileOpen(false)}>
              <BrandLogo />
              <div className="min-w-0 leading-none">
                <div className="flex items-center gap-1">
                  <span className="text-[20px] 2xl:text-[23px] font-black tracking-tight text-white">SPA</span>
                  <span className="text-[15px] 2xl:text-[17px] font-black text-teal-200">do</span>
                </div>
                <div className="text-[22px] 2xl:text-[25px] font-black tracking-tight text-white leading-[.82]">Doguinho</div>
                <div className="text-[9px] 2xl:text-[10px] text-yellow-300 mt-2 font-black whitespace-nowrap">Banho • Tosa • Veterinária</div>
              </div>
            </Link>

            <nav className="hidden xl:flex flex-1 items-center justify-center min-w-0">
              <div className="flex items-center gap-1 rounded-[20px] bg-black/18 border border-white/8 px-2 py-1.5 shadow-inner">
                {navLinks.map((item) => {
                  const Icon = item.icon;
                  return (
                    <NavLink key={item.to} to={item.to} className={({ isActive }) => `relative px-3 2xl:px-4 py-2 rounded-xl transition whitespace-nowrap flex items-center gap-2 text-[12px] 2xl:text-[13px] font-black ${isActive ? "text-white bg-green-500/18" : "text-white/66 hover:text-white hover:bg-white/7"}`}>
                      {({ isActive }) => (<><Icon size={13} className={isActive ? "text-green-300" : "text-white/45"} /><span>{item.label}</span>{isActive && <span className="absolute left-4 right-4 -bottom-1.5 h-[2px] rounded-full bg-green-300 shadow-[0_0_18px_rgba(74,222,128,.95)]" />}</>)}
                    </NavLink>
                  );
                })}
              </div>
            ...
            <div className="hidden lg:flex items-center gap-2 shrink-0 justify-end w-[300px] xl:w-[315px] 2xl:w-[345px]">
              <Link to="/agendamento" className="bg-gradient-to-br from-green-500 to-emerald-700 hover:from-green-400 hover:to-emerald-600 px-3.5 py-2.5 rounded-xl shadow-lg shadow-green-900/25 font-black flex items-center gap-2 transition border border-green-300/20 whitespace-nowrap text-[13px]"><CalendarDays size={15} /> Agendar</Link>
              <Link to="/cliente-login" className="bg-black/18 hover:bg-white/10 px-3.5 py-2.5 rounded-xl border border-yellow-400/25 font-black flex items-center gap-2 transition text-yellow-50 whitespace-nowrap text-[13px]"><User size={15} /> Cliente</Link>
              <Link to="/login" className="bg-black/18 hover:bg-white/10 px-3.5 py-2.5 rounded-xl border border-yellow-400/25 font-black flex items-center gap-2 transition text-yellow-50 whitespace-nowrap text-[13px]"><ShieldCheck size={15} /> Admin</Link>
            </div>
